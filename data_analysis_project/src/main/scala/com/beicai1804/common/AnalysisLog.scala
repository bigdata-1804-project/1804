...
import com.beicai1804.constants.LogConstants
import com.beicai1804.bean.caseclass.IPRule
import com.beicai1804.constants.LogConstants
...
object AnalysisLog {
  ...
  /**
    ...
    */
  private def handleRequestParams(logMap: mutable.Map[String, String], requestParams: String) = {
    val fields = requestParams.split("[?]")
    if (fields.length == 2 && StringUtils.isNotBlank(fields(1))) {
      val paramsText = fields(1)
      //items==>Array("en=e_pv","uid=alfjda",.....)
  ...
  def analysisLog(logText: String, ipRuleArray: Array[IPRule]) = {
    ...
    if (StringUtils.isNotBlank(logText)) { // "" null
